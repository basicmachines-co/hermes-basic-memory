...
def test_queue_prefetch_uses_longer_timeout_than_sync_prefetch(bm):
    ...
    if p._prefetch_thread:
        p._prefetch_thread.join(timeout=5.0)
    timeout = p._actor.call.call_args.kwargs.get("timeout") or p._actor.call.call_args[1].get("timeout")
...
def test_format_prefetch_handles_extra_unknown_fields(bm):
    ...
    payload = json.dumps({"results": [
        {"title": "T", "permalink": "p/t", "content": "c",
         "future_field_42": {"nested": "value"}, "score": 0.9},
    ]})
    out = _format(bm, payload)
    assert "**T**" in out
