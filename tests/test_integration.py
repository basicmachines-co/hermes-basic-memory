...
def test_bm_write_returns_full_permalink(provider, bm):
    ...
    raw = provider.handle_tool_call("bm_write", {
        "title": title,
        "content": f"# {title}\n\nbody.\n",
        "folder": "tests",
        "tags": ["integration"],
...
def test_bm_read_round_trips_a_written_note(provider, bm):
    ...
    raw = provider.handle_tool_call("bm_write", {
        "title": title, "content": body, "folder": "tests",
    })
    permalink = bm._extract_permalink(raw, "")
...
def test_bm_edit_append_lands_in_note(provider, bm):
    ...
    provider.handle_tool_call("bm_edit", {
        "identifier": permalink, "operation": "append", "content": f"\n{marker}\n",
    })
    raw = provider.handle_tool_call("bm_read", {"identifier": permalink})
    d = _parse_tool_result(raw)
    text = (d or {}).get("text") or json.dumps(d or {})
...
def test_bm_search_finds_a_freshly_written_note(provider, bm):
    ...
    provider.handle_tool_call("bm_write", {
        "title": title, "content": f"# {title}\nbody.\n", "folder": "tests",
    })
...
def test_bm_context_returns_results(provider, bm):
    ...
    permalink = bm._extract_permalink(raw, "")
    raw = provider.handle_tool_call("bm_context", {
        "url": f"memory://{permalink}", "depth": 1,
    })
    d = _parse_tool_result(raw)
...
def test_bm_move_relocates_note(provider, bm):
    ...
    raw = provider.handle_tool_call("bm_move", {
        "identifier": permalink, "new_folder": "tests/archive",
    })
    d = _parse_tool_result(raw)
    ...
    assert "tests/archive" in move_text, (
        f"move response missing new folder: {move_text[:200]}"
    )
...
def test_on_session_end_writes_summary_with_relations(provider, bm):
    ...
    raw = provider.handle_tool_call("bm_search", {
        "query": "Hermes Session Summary", "limit": 5,
    })
    d = _parse_tool_result(raw)
    text = (d or {}).get("text") or json.dumps(d or {})
